...
/*
 * We reuse the format of redisdriver.py from py-tpcch
 * File contains:
...
/*
 * tpcch has a few tables and columns:
 */
enum table { WAREHOUSE, DISTRICT, ITEM, CUSTOMER, HISTORY, STOCK, ORDERS, NEW_ORDER, ORDER_LINE, CUSTOMER_INDEXES, ORDERS_INDEXES };

size_t D_NEXT_O_ID = CUSTOMERS_PER_DISTRICT;
enum column {
   W_ID, W_NAME, W_STREET_1, W_STREET_2, W_CITY , W_STATE, W_ZIP, W_TAX, W_YTD,
   D_ID, D_W_ID, D_NAME, D_STREET_1, D_STREET_2, D_CITY, D_STATE, D_ZIP, D_TAX, D_YTD, _D_NEXT_O_ID,
...
//static long get_order_from_new_order_key(long key) {
//struct tpcch_key k = { .key = key };
//return k.prim_key & ((1LU << 32LU) - 1LU);
//}
...
static enum table get_table_from_item(char *item) {
   struct tpcch_key *k = (void*)&item[sizeof(struct item_metadata)];
   return k->table;
}
...
/* 'WAREHOUSE' elements */
static char *tpcch_warehouse(uint64_t uid) {
...
/* 'DISTRICTS' elements */
static char *tpcch_district(uint64_t warehouse, uint64_t uid) {
   ...
   item = add_shash_uint(item, _D_NEXT_O_ID, CUSTOMERS_PER_DISTRICT, 1);

   return item;
}

/* 'CUSTOMER' */
static char *tpcch_customer(uint64_t warehouse, uint64_t district, uint64_t uid) {
...
static char *tpcch_customer_index(uint64_t warehouse, uint64_t district, uint64_t uid) {
...
/* 'HISTORY' */
static char *tpcch_history(uint64_t uid) {
...
/* 'ORDER' */
static char *tpcch_order(uint64_t warehouse, uint64_t district, uint64_t client, uint64_t uid, uint64_t nb_order_line) {
...
/* 'ORDER' */
static char *tpcch_order_index(uint64_t warehouse, uint64_t district, uint64_t client, uint64_t uid) {
...
/* 'NEW_ORDER' */
static char *tpcch_new_order(uint64_t warehouse, uint64_t district, uint64_t uid) {
...
/* 'ORDER LINE' */
static char *tpcch_order_line(uint64_t warehouse, uint64_t district, uint64_t order, uint64_t uid) {
...
/* 'STOCK' */
static char *tpcch_stock(uint64_t warehouse, uint64_t uid) {
...
/* 'ITEMS' elements */
static char *tpcch_item(uint64_t uid) {
 ...
 * For this to work, max_uid should be == get_db_size_tpcch()
 */
static char *create_unique_item_tpcch(uint64_t uid, uint64_t max_uid) {
   if(uid == 0)
      assert(max_uid == get_db_size_tpcch()); // set .nb_items_in_db = get_db_size_tpcch() in main.c otherwise the DB will be partially populated

   if(uid < NB_WAREHOUSES)
      return tpcch_warehouse(uid);
   uid -= NB_WAREHOUSES;
   ...
   if(uid < NB_WAREHOUSES * DISTRICTS_PER_WAREHOUSE) {
      ...
      return tpcch_district(warehouse, district);
   }
   ...
   if(uid < NB_WAREHOUSES * DISTRICTS_PER_WAREHOUSE * CUSTOMERS_PER_DISTRICT) {
      ...
      return tpcch_customer(warehouse, district, customer);
   }
   ...
   if(uid < NB_WAREHOUSES * DISTRICTS_PER_WAREHOUSE * CUSTOMERS_PER_DISTRICT) {
      ...
      uint64_t district = (uid / CUSTOMERS_PER_DISTRICT) % DISTRICTS_PER_WAREHOUSE;
      uint64_t warehouse = (uid / CUSTOMERS_PER_DISTRICT) / DISTRICTS_PER_WAREHOUSE;
      return tpcch_customer_index(warehouse, district, customer);
   }
   uid -= NB_WAREHOUSES * DISTRICTS_PER_WAREHOUSE * CUSTOMERS_PER_DISTRICT;

   if(uid < NB_WAREHOUSES * DISTRICTS_PER_WAREHOUSE * CUSTOMERS_PER_DISTRICT) {
      return tpcch_history(uid);
   }
   uid -= NB_WAREHOUSES * DISTRICTS_PER_WAREHOUSE * CUSTOMERS_PER_DISTRICT;
   ...
   if(uid < NB_WAREHOUSES * DISTRICTS_PER_WAREHOUSE * CUSTOMERS_PER_DISTRICT) {
      ...
      uint64_t district = (uid / CUSTOMERS_PER_DISTRICT) % DISTRICTS_PER_WAREHOUSE;
      uint64_t warehouse = (uid / CUSTOMERS_PER_DISTRICT) / DISTRICTS_PER_WAREHOUSE;
      return tpcch_order(warehouse, district, client, order, AVG_OL_CNT);
   }
   uid -= NB_WAREHOUSES * DISTRICTS_PER_WAREHOUSE * CUSTOMERS_PER_DISTRICT;
   ...
   if(uid < NB_WAREHOUSES * DISTRICTS_PER_WAREHOUSE * CUSTOMERS_PER_DISTRICT) {
      ...
      uint64_t district = (uid / CUSTOMERS_PER_DISTRICT) % DISTRICTS_PER_WAREHOUSE;
      uint64_t warehouse = (uid / CUSTOMERS_PER_DISTRICT) / DISTRICTS_PER_WAREHOUSE;
      return tpcch_order_index(warehouse, district, client, order);
   }
   uid -= NB_WAREHOUSES * DISTRICTS_PER_WAREHOUSE * CUSTOMERS_PER_DISTRICT;

   if(uid < NB_WAREHOUSES * DISTRICTS_PER_WAREHOUSE * INITIAL_NEW_ORDERS_PER_DISTRICT) {
      uint64_t order = uid % INITIAL_NEW_ORDERS_PER_DISTRICT; // must be equal to client in tpcch_order, see before
      ...
      return tpcch_new_order(warehouse, district, order);
   }
   ...
   if(uid < NB_WAREHOUSES * DISTRICTS_PER_WAREHOUSE * CUSTOMERS_PER_DISTRICT * AVG_OL_CNT) {
      ...
      return tpcch_order_line(warehouse, district, order, order_line);
   }
   ...
   if(uid < NB_WAREHOUSES * NUM_ITEMS) {
      ...
      return tpcch_stock(warehouse, item);
   }
   uid -= NB_WAREHOUSES * NUM_ITEMS;

   return tpcch_item(uid);
}
...
/*
 * Generic callback helpers. Allow to automatically perform the multiple steps of a tpcch query without dealing too much with asynchrony.
 */
...
static void compute_stats_tpcch(struct slab_callback *cb, void *item) {
   struct transaction_payload *p = get_payload(cb->transaction);
...
static struct transaction_payload *get_tpcch_payload(struct transaction *t) {
   return get_payload(t);
}

static struct slab_callback *tpcch_cb(struct injector_queue *q, struct transaction *t, void *payload, slab_cb_t *next) {
   struct slab_callback *cb = new_slab_callback();
   cb->cb = compute_stats_tpcch;
   cb->injector_queue = q;
   cb->payload = payload;
   cb->payload2 = next;
   get_tpcch_payload(t)->nb_requests_to_do++;
   return cb;
}
...
static void _tpcch_status_4(struct slab_callback *cb, void *item) {
   struct status_payload *p = cb->payload;
   free(p);
}

static void _tpcch_status_3(struct slab_callback *cb, void *item) {
   struct status_payload *p = cb->payload;
   if(!item) {
      //printf("Couldn't find ORDER\n");
      return;
   }
   ...
   for(size_t i = 0; i < nb_elements; i++) {
      ...
      struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, new_payload, _tpcch_status_4);
...
static void _tpcch_status_2(struct slab_callback *cb, void *item) {
   struct status_payload *p = cb->payload;
   if(!item) {
      //printf("Couldn't find ORDERS_INDEXES\n");
      return;
   }
   ...
   for(size_t i = 0; i < s->nb_elements; i++) {
      ...
      struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, new_payload, _tpcch_status_3);
...
static void _tpcch_status_1(struct slab_callback *cb, void *item) {
   struct status_payload *p = cb->payload;
   if(!item) {
      //printf("Couldn't find CUSTOMER\n");
      return;
   }
   ...
   if(t == CUSTOMER_INDEXES) { // customer was searched by name, we now have its ID, so perform the actual query to get its data
      struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, p, _tpcch_status_1);
      ncb->item = create_key(get_key_customer(p->warehouse, p->district, p->customer));
      kv_trans_read(cb->transaction, ncb);
      return;
   }

   struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, p, _tpcch_status_2);
   ncb->item = create_key(get_key_order_index(p->warehouse, p->district, p->customer));
   kv_trans_read(cb->transaction, ncb);
}

static void tpcch_orderstatus_query(struct injector_queue *q, struct transaction *t) {
   ...
   long key;
   struct slab_callback *ncb = tpcch_cb(q, t, p, _tpcch_status_1);
...
static void _tpcch_delivery_6(struct slab_callback *cb, void *item) {
   struct delivery_payload *p = cb->payload;
   free(p);
}

static void _tpcch_delivery_5(struct slab_callback *cb, void *item) {
   struct delivery_payload *p = cb->payload;
   if(!item) {
      //printf("Couldn't find CUSTOMER.%ld.%ld.%ld\n", p->warehouse, p->district, p->customer);
      return;
   }

   char *new_item = clone_item(item);
   incr_shash_uint(new_item, C_BALANCE, p->total_sum);
   struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, p, _tpcch_delivery_6);
   ncb->item = new_item;
   kv_trans_write(cb->transaction, ncb);
}

static void _tpcch_delivery_4(struct slab_callback *cb, void *item) {
   struct delivery_payload *p = cb->payload;
   free(p);
}

static void _tpcch_delivery_3(struct slab_callback *cb, void *item) {
   struct delivery_payload *p = cb->payload;
   if(!item) {
      //printf("Couldn't find ORDER_LINE.%ld.%ld.%ld.%ld in _tpcch_delivery_3\n", p->warehouse, p->district, p->oid, p->onum);
      return;
   }
   ...
   if(p->main_payload->total_completed == p->main_payload->total_to_complete) {
      struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, p->main_payload, _tpcch_delivery_5);
      ncb->item = create_key(get_key_customer(p->warehouse, p->district, p->customer));
      kv_trans_read(cb->transaction, ncb);
   }
   ...
   struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, p, _tpcch_delivery_4);
   ncb->item = new_item;
   kv_trans_write(cb->transaction, ncb);
}

static void _tpcch_delivery_2(struct slab_callback *cb, void *item) {
   // Nothing to do, we updated the order
}

static void _tpcch_delivery_1(struct slab_callback *cb, void *item) {
   struct delivery_payload *p = cb->payload;
   if(!item) {
      //printf("Couldn't find ORDER.%ld.%ld.%ld\n", p->warehouse, p->district, p->oid);
      return;
   }
   ...
   struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, p, _tpcch_delivery_2);
   ncb->item = new_item;
   kv_trans_write(cb->transaction, ncb);
   ...
   for(size_t i = 0; i < nb_elements; i++) {
      ...
      struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, new_payload, _tpcch_delivery_3);
      ncb->item = create_key(key_order_line);
      kv_trans_read(cb->transaction, ncb);
   }
}

static void tpcch_delivery_query(struct injector_queue *q, struct transaction *t) {
...
static void _tpcch_new_order_7(struct slab_callback *cb, void *item) {
   struct new_order_payload *p = cb->payload;
   free(p);
}

// Add order_line
static void _tpcch_new_order_6(struct slab_callback *cb, void *item) {
   struct new_order_payload *p = cb->payload;

   char *new_order_line = tpcch_order_line(p->warehouse, p->district, p->oid, p->item_id);
   struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, p, _tpcch_new_order_7);
   ncb->item = new_order_line;
   kv_trans_write(cb->transaction, ncb);
}

// Update stock
static void _tpcch_new_order_5(struct slab_callback *cb, void *item) {
   struct new_order_payload *p = cb->payload;
   if(!item) {
      //printf("Cannot read STOCK.%ld.%ld\n", p->warehouse, p->items[p->item_id].iid);
      return;
   }

   char *new_item = clone_item(item);
   new_item = incr_shash_uint(new_item, S_QUANTITY, p->items[p->item_id].quantity); // This is not what tpcch does, but it's the same stuff from the DB point of view

   struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, p, _tpcch_new_order_6);
   ncb->item = new_item;
   //kv_trans_write(cb->transaction, ncb); // too many aborts!
   kv_trans_read(cb->transaction, ncb);
}

static void _tpcch_new_order_4b(struct slab_callback *cb, void *item) {
   struct new_order_payload *p = cb->payload;
   free(p);
}

// Get stocks of all items
static void _tpcch_new_order_4(struct slab_callback *cb, void *item) {
   struct new_order_payload *p = cb->payload;
   ...
   for(size_t i = 0; i < p->ol_cnt; i++) {
      ...
      struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, new_payload, _tpcch_new_order_5);
...
static void _tpcch_new_order_3b(struct slab_callback *cb, void *item) {
   struct new_order_payload *p = cb->payload;
   ...
   if(item) {
      ...
      new_item = tpcch_order_index(p->warehouse, p->district, p->customer, p->oid);
   }

   struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, p, _tpcch_new_order_4b);
   ncb->item = new_item;
   kv_trans_write(cb->transaction, ncb);
}

// Create the new order
static void _tpcch_new_order_3(struct slab_callback *cb, void *item) {
   struct new_order_payload *p = cb->payload;

   char *new_order = tpcch_new_order(p->warehouse, p->district, p->oid);
   struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, p, _tpcch_new_order_4);
   ncb->item = new_order;
   kv_trans_write(cb->transaction, ncb);
}

// Add the order in the order table and query the order index of the customer
static void _tpcch_new_order_2(struct slab_callback *cb, void *item) {
   struct new_order_payload *p = cb->payload;

   struct new_order_payload *new_payload = malloc(sizeof(*p));
   memcpy(new_payload, p, sizeof(*p));

   char *order = tpcch_order(p->warehouse, p->district, p->customer, p->oid, p->ol_cnt);
   struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, p, _tpcch_new_order_3);
   ncb->item = order;
   kv_trans_write(cb->transaction, ncb);

   struct slab_callback *ncb2 = tpcch_cb(cb->injector_queue, cb->transaction, new_payload, _tpcch_new_order_3b);
...
static void _tpcch_new_order_1(struct slab_callback *cb, void *item) {
   struct new_order_payload *p = cb->payload;
   if(!item) {
      //printf("Couldn't find DISTRICT.%ld.%ld\n",  p->warehouse, p->district);
      return;
   }

   p->oid = __sync_add_and_fetch(&D_NEXT_O_ID, 1);

   char *new_item = clone_item(item);
   incr_shash_uint(new_item, _D_NEXT_O_ID, 1);

   struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, p, _tpcch_new_order_2);
   ncb->item = new_item;
   kv_trans_read(cb->transaction, ncb);
   //kv_trans_write(cb->transaction, ncb); // too many aborts!
}

static void tpcch_new_order_query(struct injector_queue *q, struct transaction *t) {
...
again:
   ...
   struct slab_callback *ncb = tpcch_cb(q, t, p, _tpcch_new_order_1);
...
static void _tpcch_payment_7(struct slab_callback *cb, void *item) {
   // Payment done
   struct payment_payload *p = cb->payload;
   free(p);
}

static void _tpcch_payment_6(struct slab_callback *cb, void *item) {
   ...
   // We don't do that to avoid write conflicts in transactions ('HISTORY' has no primary key in tpcch so that's OK)
   struct payment_payload *p = cb->payload;
   struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, p, _tpcch_payment_7);
   ncb->item = tpcch_history(uniform_next());
   kv_trans_write(cb->transaction, ncb);
}

static void _tpcch_payment_5(struct slab_callback *cb, void *item) {
   // Read client info
   struct payment_payload *p = cb->payload;
   if(!item) {
      //printf("Couldn't find CUSTOMER\n");
   ...
   enum table t = get_table_from_item(item);
   if(t == CUSTOMER_INDEXES) { // customer was searched by name, we now have its ID, so perform the actual query to get its data
      struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, p, _tpcch_payment_5);
      ncb->item = create_key(get_key_customer(p->warehouse, p->district, p->customer));
      kv_trans_read(cb->transaction, ncb);
      return;
   }
   ...
   struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, p, _tpcch_payment_6);
   ncb->item = new_item;
   kv_trans_write(cb->transaction, ncb);
}

static void _tpcch_payment_4(struct slab_callback *cb, void *item) {
...
static void _tpcch_payment_3(struct slab_callback *cb, void *item) {
   //struct payment_payload *p = cb->payload;
   if(!item) {
      //printf("Couldn't find DISTRICT.%ld.%ld\n", p->warehouse, p->district);
      return;
   }
   _tpcch_payment_4(cb, item);
}


static void _tpcch_payment_2(struct slab_callback *cb, void *item) {
   // Update of the WAREHOUSE has been done, now read the district
   struct payment_payload *p = cb->payload;

   struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, p, _tpcch_payment_3);
   ncb->item = create_key(get_key_district(p->warehouse, p->district));
   kv_trans_read(cb->transaction, ncb);
}

static void _tpcch_payment_1(struct slab_callback *cb, void *item) {
   //struct payment_payload *p = cb->payload;
   if(!item) {
      //printf("Couldn't find WAREHOUSE.%ld\n", p->warehouse);
      return;
   }
   _tpcch_payment_2(cb, item);
}


static void tpcch_payment_query(struct injector_queue *q, struct transaction *t) {
   ...
   struct slab_callback *ncb = tpcch_cb(q, t, p, _tpcch_payment_1);
...
static void _tpcch_stock_level_4(struct slab_callback *cb, void *item) {
   struct stock_payload *p = cb->payload;
   if(!item) {
      //printf("Couldn't find STOCK.%ld.%ld\n",  p->warehouse, p->iid);
      return;
   }
...
static void _tpcch_stock_level_3(struct slab_callback *cb, void *item) {
   struct stock_payload *p = cb->payload;
   if(!item) {
      //printf("Couldn't find ORDER_LINE.%ld.%ld.%ld.%ld in _tpcch_stock_level_3\n",  p->warehouse, p->district, p->oid, p->onum);
      return;
   }
   ...
   struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, p, _tpcch_stock_level_4);
   ncb->item = create_key(stock_key);
   kv_trans_read(cb->transaction, ncb);
}

static void _tpcch_stock_level_2(struct slab_callback *cb, void *item) {
   struct stock_payload *p = cb->payload;
   if(!item) {
      //printf("Couldn't find ORDER.%ld.%ld.%ld\n",  p->warehouse, p->district, p->oid);
      return;
   ...
   for(size_t i = 0; i < nb_elements; i++) {
      ...
      struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, new_payload, _tpcch_stock_level_3);
      ncb->item = create_key(key_order_line);
      kv_trans_read(cb->transaction, ncb);
   }
...
static void _tpcch_stock_level_1(struct slab_callback *cb, void *item) {
   struct stock_payload *p = cb->payload;
   if(!item) {
      //printf("Couldn't find DISTRICT.%ld.%ld\n",  p->warehouse, p->district);
      return;
   ...
   long oid = get_shash_uint(item, _D_NEXT_O_ID) - 1;
   ...
   for(size_t i = 0; i < NB_STOCK_ITEMS; i++) {
      ...
      long order_line_key = get_key_order(p->warehouse, p->district, new_payload->oid);
      struct slab_callback *ncb = tpcch_cb(cb->injector_queue, cb->transaction, new_payload, _tpcch_stock_level_2);
      ncb->item = create_key(order_line_key);
      kv_trans_read(cb->transaction, ncb);
   }
...
static void tpcch_stock_level_query(struct injector_queue *q, struct transaction *t) {
   ...
   struct slab_callback *ncb = tpcch_cb(q, t, p, _tpcch_stock_level_1);
...
static void _launch_tpcch(struct workload *w, int test, int nb_requests, int zipfian) {
   ...
   if(uid == 0)
      _p.scan_cb = start_background_scan(uid, w, q, get_key_order_line(0, 0, 0, 0), get_key_order_line(10, DISTRICTS_PER_WAREHOUSE, CUSTOMERS_PER_DISTRICT, AVG_OL_CNT));

   declare_periodic_count;
   size_t nb_queries = 0;
   while(glob_nb_scans_done != NB_BG_SCANS) {
      ...
      if(x <= 4) {
         p->query_id = 0;
         tpcch_stock_level_query(q, t);
      } else if(x <= 4 + 4) {
         p->query_id = 1;
         tpcch_delivery_query(q, t);
      } else if (x <= 4 + 4 + 4) {
         p->query_id = 2;
         tpcch_orderstatus_query(q, t);
      } else if (x <= 43 + 4 + 4 + 4) {
         p->query_id = 3;
         tpcch_payment_query(q, t);
      } else {
         p->query_id = 4;
         tpcch_new_order_query(q, t);
      }
      nb_queries++;

      periodic_count(1000, "TRANS Load Injector %d [failure rate = %lu%%] [snapshot size %lu] [running transactions %lu] [%lu scans done - %d full] [%lu queries done] [%lu db size]", uid, failed_trans*100/nb_commits_done, get_snapshot_size(), get_nb_running_transactions(), scan_progress(_p.scan_cb), glob_nb_scans_done, nb_queries, get_database_size());
...
/* Generic interface */
static void launch_tpcch(struct workload *w, bench_t b) {
   return _launch_tpcch(w, 0, w->nb_requests_per_thread, 0);
}

/* Pretty printing */
static const char *name_tpcch(bench_t w) {
   printf("tpcch BENCHMARK [failure rate = %f%%, %lu commits done, %lu max parallel transactions] [snapshot size %lu]\n", ((double)failed_trans)*100./(double)nb_commits_done, nb_commits_done, get_max_recorded_parallel_transactions(), get_snapshot_size());
   return "tpcch";
}

static int handles_tpcch(bench_t w) {
...
static const char* api_name_tpcch(void) {
   return "tpcch";
}

struct workload_api TPCCH = {
   .handles = handles_tpcch,
   .launch = launch_tpcch,
   .api_name = api_name_tpcch,
   .name = name_tpcch,
   .create_unique_item = create_unique_item_tpcch,
};
